...
export interface HeadConfigInterface { hidden?:boolean, color?: string, background?: string, fontSize?: number, heigth?: number, border?: number | boxCoordinatesInterface | pointCoordinatesInterface, borderColor?: string }
...
export interface addTableInterface { data: any[], width?: widthInterface | number, fontSize: number, color: string, margin?: number | boxCoordinatesInterface | pointCoordinatesInterface, settingsHead?: HeadConfigInterface, cellHeight?: number, settingsItems?: ItemConfigInterface[], border?: number | boxCoordinatesInterface | pointCoordinatesInterface, borderColor?: string, viewTotal?: viewTotalConfigInterface }
...
export class GeneratorReportPdfV2 {
   ...
   public addTable(tableSettings:addTableInterface) {
      if (tableSettings.data.length == 0) return;
      this.temporal_value = ''
      var height = 15 + (tableSettings.cellHeight ?? 0)
      var margin = this.getSpacingElement(tableSettings.margin)
      var border = this.getSpacingElement(tableSettings.border)
      var totals = {}
      var configSubHead = tableSettings.settingsItems?.find((s: ItemConfigInterface) => s.subHead != null)
      ...
      var table = this.createTable(tableSettings.data[0], tableSettings.fontSize, height, tableSettings.color, margin, border, tableSettings.borderColor, tableSettings.settingsHead, tableSettings.settingsItems, tableSettings.width)

      if (configSubHead){
         var head_property = configSubHead.property ?? ''
         var heads = tableSettings.data.reduce((acc: any, item: any) => { var exists = acc.find((a: any) => a == item[head_property]); if (!exists) acc.push(item[head_property]); return acc }, [])
         if(heads.length > 1) this.tableSubHead(table, tableSettings.data[0], height, tableSettings.fontSize, tableSettings.color, configSubHead)
         else configSubHead = undefined 
      }

      for (let i = 0; i < tableSettings.data.length; i++) {
         var row = this.tableRow(tableSettings.data[i], height, tableSettings.fontSize, tableSettings.color, tableSettings.settingsItems, tableSettings.viewTotal?.propertys)
         table.appendChild(row)
         if ((i + 1) < tableSettings.data.length && (!configSubHead && (this.stateHeigthPage + height) > this.HeightPage) || (configSubHead && (this.stateHeigthPage + height + height) > this.HeightPage)) {
            this.addPage(this.margin)
            table = this.createTable(tableSettings.data[i + 1], tableSettings.fontSize, height, tableSettings.color, margin, border, tableSettings.borderColor, tableSettings.settingsHead, tableSettings.settingsItems)
            if (configSubHead) this.tableSubHead(table, tableSettings.data[i + 1], height, tableSettings.fontSize, tableSettings.color, configSubHead)
         }
         if (configSubHead && (i + 1) < tableSettings.data.length && this.temporal_value != tableSettings.data[i + 1][configSubHead.property]) this.tableSubHead(table, tableSettings.data[i + 1], height, tableSettings.fontSize, tableSettings.color, configSubHead)
      }
      if (tableSettings.viewTotal) {
         if ((this.stateHeigthPage + height) > this.HeightPage) {
            this.addPage(this.margin)
            table = this.createTable(tableSettings.data[0], tableSettings.fontSize, height, tableSettings.color, margin, border, tableSettings.borderColor, tableSettings.settingsHead, tableSettings.settingsItems)
         }
         var tr = this.CreatorElement('tr')
         var text_title = ''
         var border_total = this.getSpacingElement(tableSettings.viewTotal.border)
         if(!tableSettings.viewTotal.show_total_records && tableSettings.viewTotal.title) text_title = tableSettings.viewTotal.title
         else if(tableSettings.viewTotal.show_total_records && !tableSettings.viewTotal.title) text_title = 'Total de registros ' + tableSettings.data.length
         else if(tableSettings.viewTotal.show_total_records && tableSettings.viewTotal.title) text_title = tableSettings.viewTotal.title + '  (' + 'Total de registros ' +tableSettings. data.length + ')'
         tr.style.height = height + 'px'
         tr.style.maxHeight = height + 'px'
         tr.style.minHeight = height + 'px'
         tr.style.overflow = 'hidden'
         tr.style.background = tableSettings.viewTotal.background ?? 'transparent'
         tr.style.color = tableSettings.viewTotal.color ?? tableSettings.color
         tr.style.borderTop = (border_total.top ?? 0) + 'px solid ' + (tableSettings.viewTotal?.borderColor ?? '#000000')
         tr.style.borderLeft = (border_total.left ?? 0) + 'px solid ' + (tableSettings.viewTotal?.borderColor ?? '#000000')
         tr.style.borderRight = (border_total.right ?? 0) + 'px solid ' + (tableSettings.viewTotal?.borderColor ?? '#000000')
         tr.style.borderBottom = (border_total.bottom ?? 0) + 'px solid ' + (tableSettings.viewTotal?.borderColor ?? '#000000')
         this.stateHeigthPage += (height + border_total.top + border_total.bottom)
         var temporal_td: any
         var firsh_element = true
         var old_text = false
         for (let key in tableSettings.data[0]) {
            var setting = tableSettings.settingsItems?.find((s: ItemConfigInterface) => s.property == key)
            if (!setting || setting && !setting?.subHead || setting?.subHead?.showItem) {
               var td: any = this.CreatorElement('td')
               td.style.height = height + 'px'
               td.style.fontSize = (tableSettings.viewTotal.fontSize ?? (tableSettings.fontSize - 1)) + 'px'
               td.style.fontWeight = 'bold'
               td.style.color = tableSettings.viewTotal.color ?? tableSettings.color
               var exists = tableSettings.viewTotal.propertys?.find((p: totalPropertysInterface) => p.proeprty == key)
               if(exists){
   ...
   // METODOS PRIVADOS
   private createTable(data: any, fontSize: number, height: number, color: string, margin: positionInterface, border: positionInterface, borderColor?: string, settingsHead?: HeadConfigInterface, settingsItems?: ItemConfigInterface[], width?: number | widthInterface ): HTMLElement {
      var table = this.CreatorElement('table')
      var size_width = (this.WidthPage - margin.left - margin.right) + 'px'
      if(width) {
         if(typeof width === 'number') var size_width = (width - margin.left - margin.right) + 'px'
         else size_width = width
      }
      table.style.width = size_width
      table.style.maxWidth = size_width
      table.style.minWidth = size_width
