...
export class HomeService {
  ...
  alert = (data:AlertInterface)=> new Promise<any>((resolve, reject)=>{
    const dialogRef = this.dialog.open(AlertComponent, {data: data, 'width' : (data.width ? data.width : '320')+'px', panelClass: 'alert-container', disableClose:data?.hiddenButtonCancel});
  ...
  tGenerate(cant: number) {
    let caracteres = "abcdefghijkmnpqrtuvwxyzABCDEFGHJKMNPQRTUVWXYZ123467890";
    let token = "";
