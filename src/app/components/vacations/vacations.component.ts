...
export class VacationsComponent {
  ...
  delete_v(vacation:any){
    this.home.alert({icon: 'warning', title: '¿Está seguro de eliminar esta solicitud de vacaciones?', text: 'Esta acción no se puede deshacer'}).then((result:any)=>{
      this.loading = true
      this.http.post('vacation/asing', {action: 'delete', period:0, userid: vacation.id_user, start: vacation.desde, end: vacation.hasta}).then((res:any)=>{
        this.load()
        this.home.toast.fire({icon: 'success', title: 'Solicitud de vacaciones eliminada'})
      }).catch((err:any)=>{
        this.loading = false
      })
    }).catch((err:any)=>{
  ...
  add(){
    this.dialog.open(VacationsAddComponent, {data: {}, width: '340px', disableClose: true, panelClass: 'asignUser'})
