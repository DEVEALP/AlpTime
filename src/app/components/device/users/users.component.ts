...
export class UsersComponent {
  ...
  ngOnInit() {
    ...
    this.load();
    if(this.devicesvc.view_user_biometric) this.view_user(this.devicesvc.view_user_biometric)
  }

  ngOnDestroy() {
    this.devicesvc.view_user_biometric = undefined
  }
  ...
  view_user(data?: any) {
    if(data){
      this.homesvc.loading.emit(true)
      this.http.post('device/users/get', {top: 0, search:data.userid, deviceid: this.device.id, type:'user' }).then((resp:any)=>{
        this.homesvc.loading.emit(false)
        if(resp.length > 0){
          this.setUser(resp[0])
        }else{
