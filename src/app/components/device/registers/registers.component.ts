...
export class RegistersComponent {
  ...
  constructor(private router_actived: ActivatedRoute, private router: Router, private homesvc: HomeService, private dialog: MatDialog, private http: newHttpRequest, private devicesvc:DeviceService) { }
  ...
  ngOnInit() {
    ...
    if(this.devicesvc.view_records){
      this.start.setValue(new Date(this.devicesvc.view_records.date))
      this.end.setValue(new Date(this.devicesvc.view_records.date))
      this.filters.user = this.devicesvc.view_records.userid
    }
    this.load();
    this.getDataFilter();
  }
  ...
  ngOnDestroy() {
    this.devicesvc.view_records = undefined
  }
  ...
  setRecord(data?: any) {
    ...
    const dialogRef = this.dialog.open(RecordComponent, { data: { record: data, deviceid: this.device.id, user: this.filters.user }, panelClass: 'userTurns', width: '350px', maxWidth: '90vw' });
