...
export class RecordComponent {
  ...
  getUsers(){
    this.loading = true
    this.http.post('device/users/get', { type:'min', deviceid:0, search: this.user.value, top: 5}).then((data: any) => {
      this.users = data
  ...
  save(){
    this.valid()
    if(this.seletedUser.userid){
      var date = this.year + '-' + this.month + '-' + this.day + ' ' + this.hour?.toString().padStart(2, '0') + ':' + this.minute?.toString().padStart(2, '0') + ':' + this.second?.toString().padStart(2, '0')
      var data = {datetime: date ,deviceid: this.data.deviceid, userid: this.seletedUser.userid}
