...
export class TimeReportComponent {
  ...
  constructor(private home: HomeService, private http: newHttpRequest, private turnsvc: TurnsService, private devicesvc: DeviceService, private router: Router) { }
  ...
  ngOnInit() {
    ...
    if (this.devicesvc.report_user) {
      this.loadUsers = true
      this.user = { id: 0, userid: this.devicesvc.report_user?.userid, name: '', imagen: '' }
      this.http.post('device/users/get', { type: 'id', top: 0, search: this.user.userid, deviceid: 0 }).then((data: any) => {
        if (data.length > 0) {
          this.user = data[0]
          this.user_search = this.formatearCapitalizarTexto(this.user.name)
          var start = new Date(this.devicesvc.report_user?.date_start ?? '')
          this.start.setValue(start)
          this.end.setValue(new Date(this.devicesvc.report_user?.date_end ?? ''))
  ...
  ngOnDestroy() {
    if (this.destroy) this.devicesvc.report_user = undefined
  }

  view_records(item: any, index: number,) {
    var deviceid: any;
    if (index == 1) deviceid = item.ent1_deviceid
    if (index == 2) deviceid = item.sal1_deviceid
    if (index == 3) deviceid = item.ent2_deviceid
    if (index == 4) deviceid = item.sal2_deviceid
    if (deviceid) {
      this.home.alert({ icon: 'info', title: `Desearia ver los registros del biometrico del dia ${this.formatearFecha(item.fecha)}?` }).then((e: any) => {
        var data = { userid: Number(item.userid), deviceid: deviceid, date: item.fecha?.trim() + 'T05:00', url: 'reports/time_report' }
        this.devicesvc.view_records = data
        this.devicesvc.report_user = { userid: this.user.userid, date_start: this.start.value?.toISOString() ?? '', date_end: this.end.value?.toISOString() ?? '' }
        this.destroy = false
        this.router.navigate(['devices', data.deviceid])
      })
    }
  }
  ...
  ngAfterViewInit() {
    if (!this.turnsvc.view_user_report && !this.devicesvc.report_user) document.getElementById('search')?.focus()
  }
  ...
  getDataGraph() {
    ...
    this.total_atrasos = this.sumarTiempos(this.records.filter(x => x.atraso != '00:00').map(x => x.atraso))
    ...
    this.total_permisos = this.sumarTiempos(this.records.filter(x => x.permiso != '00:00').map(x => x.permiso))
    ...
    this.total_vacaciones = this.sumarTiempos(this.records.filter(x => x.vacaciones != '00:00').map(x => x.vacaciones))
    ...
    this.total_faltas = this.sumarTiempos(this.records.filter(x => x.falta != '00:00').map(x => x.falta))
  ...
  donwload_pdf() {
    ...
    for (let x of this.records) {
      ...
      if (this.settings?.time?.column?.h25 == false) delete item.h25
      if (this.settings?.time?.column?.h50 == false) delete item.h50
      if (this.settings?.time?.column?.h100 == false) delete item.h100
    ...
    var table:addTableInterface = {data:report_table, fontSize:size, color:'#333', margin:{ top: 10 }, settingsHead: { border: { bottom: 1 }, borderColor: '#B4B4B4' }, cellHeight:heigth, settingsItems: items, border:0}
    report.addTable(table)

    var totals: any[] = []
    totals.push({ title: 'Total horas trabajadas', value: this.total_hours })
    totals.push({ title: 'Total atrasos', value: this.total_atrasos })
    totals.push({ title: 'Total horas falta', value: this.total_faltas })
    if (this.settings?.time?.column?.h25 == false) totals.push({ title: 'Total horas 25%', value: this.total_25 })
    if (this.settings?.time?.column?.h50 == false) totals.push({ title: 'Total horas 50%', value: this.total_50 })
    if (this.settings?.time?.column?.h100 == false) totals.push({ title: 'Total horas 100%', value: this.total_100 })
    var table_total:addTableInterface = {data:totals, width:200, fontSize: size, color: '#333', margin: { top: 30 }, settingsHead: {hidden:true}, cellHeight: (heigth - 3), border:1, borderColor:'#999'}
    report.addTable(table_total)
